...
final class AuthController extends AbstractController
{
    ...
    public function login(
        ...
        if ($request->isMethod("POST")) {
            ...
            $user = $em
                ->createQuery(
                    "SELECT u FROM App\Entity\Utilisateur u WHERE LOWER(u.email_u) = LOWER(:email)",
                )
            ...
            if (
                !$user ||
                !password_verify($password, $user->getMotDePasseU())
            ) {
                return $this->render("auth/login.html.twig", [
    ...
    public function forgotPassword(
        ...
        if ($request->isMethod("POST")) {
            ...
            if ($action === "start") {
                ...
                if (!$this->isCsrfTokenValid("forgot_2fa", $token)) {
                    ...
                    if (!$verified) {
                        $attempts =
                            (int) ($context["two_factor_attempts"] ?? 0) + 1;
                        ...
                        if ($attempts >= self::FORGOT_2FA_MAX_ATTEMPTS) {
                            ...
                            $context = [];
                            $error =
                                "Too many invalid 2FA attempts. Please restart the process.";
                        } else {
                            $this->setForgotPasswordContext($request, $context);
                        ...
                        $context["email_code_expires_at"] =
                            time() + self::FORGOT_EMAIL_CODE_TTL_SECONDS;
                        $context["email_code_attempts"] = 0;
                        $context["email_code_last_sent_at"] = time();
                        $this->setForgotPasswordContext($request, $context);

                        try {
                            $this->sendForgotPasswordCodeEmail(
                                $mailer,
                            ...
                            $error =
                                "2FA is valid but email sending failed. Please retry sending the code.";
                        }
                    }
                }
                ...
                if (!$this->isCsrfTokenValid("forgot_email_code", $token)) {
                    ...
                    $lastSentAt =
                        (int) ($context["email_code_last_sent_at"] ?? 0);
                    if (
                        $lastSentAt > 0 &&
                        time() - $lastSentAt <
                            self::FORGOT_EMAIL_RESEND_COOLDOWN_SECONDS
                    ) {
                        $remaining =
                            self::FORGOT_EMAIL_RESEND_COOLDOWN_SECONDS -
                            (time() - $lastSentAt);
                        ...
                        $context["email_code_hash"] = password_hash(
                            $emailCode,
                            PASSWORD_BCRYPT,
                        );
                        $context["email_code_expires_at"] =
                        ...
                        try {
                            ...
                            $error =
                                "Unable to send email now. Please try again in a moment.";
                        }
                    }
                }
                ...
                if (!$this->isCsrfTokenValid("forgot_email_code", $token)) {
                    ...
                    if ($expiresAt <= 0 || time() > $expiresAt) {
                        ...
                        $attempts =
                            (int) ($context["email_code_attempts"] ?? 0);
                        if ($attempts >= self::FORGOT_EMAIL_CODE_MAX_ATTEMPTS) {
                            $this->clearForgotPasswordContext($request);
                            $context = [];
                            $error =
                                "Too many invalid email code attempts. Please restart the process.";
                        } else {
                            $inputCode = preg_replace(
                                "/\s+/",
                                "",
                                trim(
                            ...
                            $hash =
                                (string) ($context["email_code_hash"] ?? "");
                            $isValidCode =
                                is_string($inputCode) &&
                            ...
                            if (!$isValidCode) {
                                ...
                                $success =
                                    "Email code verified. You can now set a new password.";
                            }
                        }
                    }
                }
                ...
                if (!$this->isCsrfTokenValid("forgot_reset_password", $token)) {
                    ...
                    $newPassword = (string) $request->request->get(
                        "new_password",
                        "",
                    );
                    $confirmPassword = (string) $request->request->get(
                        "confirm_password",
                        "",
                    );
        ...
        if (
            !in_array(
                $step,
                ["email", "two_factor", "email_code", "reset_password"],
                true,
            )
        ) {
            $step = "email";
    ...
    public function faceIdDetect(
        ...
        if (!is_string($image) || $image === "") {
            return new JsonResponse(
                ["ok" => false, "error" => "Missing frame data"],
                400,
            );
        }
    ...
    #[
        Route(
            "/face-id/recognize",
            name: "app_face_id_recognize",
            methods: ["POST"],
        ),
    ]
    public function faceIdRecognize(
        Request $request,
        FaceIdClient $faceIdClient,
    ): JsonResponse {
        $payload = json_decode((string) $request->getContent(), true);
    ...
    public function connectGoogleCheck(
        ...
        if (!$user) {
            ...
            $user->setMotDePasseU(password_hash("123456", PASSWORD_BCRYPT));
    ...
    public function signup(
        ...
        if ($request->isMethod("POST")) {
            ...
            $faceEmbeddingsRaw = $request->request->get(
            ...
            if (empty($password) || strlen($password) < 4 || strlen($password) > 6) {
                $errors["password"] = "Password length must be between 4 and 6 characters";
            }
            ...
            try {
                ...
                if ($fingerprintPending === "1") {
                    return new JsonResponse([
                        ...
                        "redirect" => $this->generateUrl("app_signup"),
                    ]);
                }
    ...
    public function home(EntityManagerInterface $em): Response
    {
        $produits = $em
            ...
            ->findBy(["statut" => "Disponible"], ["nom" => "ASC"]);
    ...
    public function submitComplaint(
        ...
        // Get form data
        $type_reclamation = trim(
            $request->request->get("type_reclamation", ""),
        );
        ...
        if (!$userId) {
            return new JsonResponse([
                "success" => false,
                "errors" => [
                    "general" => "You must be logged in to submit a complaint",
                ],
            ]);
        }
    ...
    private function getForgotPasswordContext(Request $request): array
    {
        $context = $request
            ->getSession()
            ->get(self::FORGOT_CONTEXT_SESSION_KEY, []);
        return is_array($context) ? $context : [];
    }

    private function setForgotPasswordContext(
        Request $request,
        array $context,
    ): void {
    ...
    private function sendForgotPasswordCodeEmail(
        MailerInterface $mailer,
        ...
        // Keep same transport strategy as complaint replies.
        if (
            $alternateMailerDsn !== "" &&
        ...
        $email->subject("EL FIRMA - Password Reset Verification Code");
        $email->html(
            '<div style="font-family: Arial, sans-serif; color: #1b1d0e; line-height: 1.6;">' .
                ...
                '<p style="font-size: 28px; font-weight: 700; letter-spacing: 4px; margin: 12px 0; color: #116530;">' .
    ...
    private function isOauthClientConfigured(
        ...
        // Block obvious placeholders to avoid redirecting users to provider-side "invalid_client" pages.
        if (
            str_starts_with($clientId, "dev-") ||
            str_starts_with($clientSecret, "dev-")
        ) {
            return false;
        }

        if (
            str_contains($clientId, "CHANGE_ME") ||
            str_contains($clientSecret, "CHANGE_ME")
        ) {
            return false;
        }
