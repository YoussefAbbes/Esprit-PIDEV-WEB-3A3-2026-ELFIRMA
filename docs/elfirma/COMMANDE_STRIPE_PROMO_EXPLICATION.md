...
## 25) Bundle geocodage + analyse geographique clients (zone la plus rentable)
### A) Objectif fonctionnel
- Ajouter un vrai bundle geocodage via Composer (demande bundle externe).
- Ajouter un bloc admin qui detecte la zone la plus rentable a partir des adresses de livraison clients.

### B) Bundle externe installe
- Package principal:
  - `willdurand/geocoder-bundle` (Bazinga Geocoder Bundle)
- Provider geocodage:
  - `geocoder-php/nominatim-provider`
- Dependances techniques ajoutees pour PSR HTTP:
  - `nyholm/psr7`
  - `psr/http-client`

### C) Configuration Symfony ajoutee
- Activation bundle:
  - `config/bundles.php`
  - `Bazinga\\GeocoderBundle\\BazingaGeocoderBundle::class => ['all' => true]`
- Configuration provider:
  - `config/packages/geocoder.yaml`
  - provider `nominatim` + `user_agent` projet.
- Configuration recipe HTTP discovery:
  - `config/packages/http_discovery.yaml`

### D) MVC applique pour l'analyse geographique
- Controller:
  - `src/Controller/CommandeController.php`
  - calcul server-side via:
    - `buildGeoZoneAnalytics(...)`
    - `extractZoneFromAddress(...)`
  - donnees injectees a Twig sous `geo_zone_analytics`.
- View:
  - `templates/elfirma/commandes.html.twig`
  - nouveau panneau admin "Analyse geographique clients" avec:
    - top zone rentable,
    - chiffre d'affaires de la zone,
    - panier moyen,
    - tableau top zones,
    - compteur adresses non classables.

### E) Regles metier de calcul
- Groupement par zone detectee depuis `adresse_livraison`.
- Dictionnaire de gouvernorats Tunisie pour normaliser la zone.
- Si non reconnu:
  - fallback sur dernier segment texte de l'adresse.
- Revenue zone:
  - somme des `prix_total` hors commandes `Annulée`.
- Classement:
  - tri par CA desc, puis nb commandes desc.

### F) Validation technique
- `php -l src/Controller/CommandeController.php` => OK
- `php bin/console lint:twig templates/elfirma/commandes.html.twig` => OK
- `php bin/console lint:container` => OK

### G) Journal des changements (suite)
- Etape 31: installation bundle geocodage `willdurand/geocoder-bundle` + provider Nominatim.
- Etape 32: activation/config bundle (`bundles.php`, `geocoder.yaml`, `http_discovery.yaml`).
- Etape 33: ajout analyse geographique admin (zone la plus rentable depuis adresses clients).
